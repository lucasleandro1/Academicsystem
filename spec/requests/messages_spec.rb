...
RSpec.describe "Messages", type: :request do
  ...
  describe "GET /messages/new" do
    context "when user is admin" do
      ...
      it "shows all users except himself as available recipients" do
        ...
        # Os recipients devem incluir todos os outros usuários
        recipients = assigns(:recipients)
        expect(recipients).to include(director, teacher, student)
  ...
  describe "POST /messages" do
    ...
    shared_examples "blocks message creation" do
      it "does not create the message and renders new template" do
        expect {
          post messages_path, params: message_params
  ...
  describe "DELETE /messages/:id" do
    ...
    it "allows user to delete their sent messages" do
      expect {
        delete message_path(sent_message)
      }.to change(Message, :count).by(-1)
      expect(response).to redirect_to(messages_path)
    end
    ...
    it "allows user to delete their received messages" do
      expect {
        delete message_path(received_message)
      }.to change(Message, :count).by(-1)
      expect(response).to redirect_to(messages_path)
    end
    ...
    it "does not allow user to delete messages they are not involved in" do
      ...
      expect {
        delete message_path(other_message)
